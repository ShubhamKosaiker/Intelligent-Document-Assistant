...
def upload_document():
    try:
        ...
        if file.filename=='':
            ...
            return jsonify({'error':'No file selescted'}),400
        
        #check extension
        if not file.filename.endswith(('.txt','.pdf')):
            logger.warning(f"Unsupported filetype : {file.filename}")
        ...
        try:
            ...
            logger.debug("Documents added in vectoe store")
        except Exception as e:
            logger.error(f"Error adding documents to vector store: {str(e)}")
            return jsonify({'error':f'Error addding documents to vector store: {str(e)}'}),500
